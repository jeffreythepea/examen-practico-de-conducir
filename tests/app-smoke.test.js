...
test('an installed offline package can be asked for updates without a relaunch', async () => {
  ...
  assert.match(source, /upToDate: offlineUpToDate/);
...
test('setup exposes bilingual offline status and download actions without blocking Start', async () => {
  ...
  assert.match(source, /renderOfflineCardView\(\{/);
...
test('a tap in flight cannot act on the offline button that replaced its target', async () => {
  ...
  assert.match(
    source,
    /if \(card\.action !== offlineActionShown\) \{\s*\n\s*offlineActionShown = card\.action;\s*\n\s*offlineActionShownAt = Date\.now\(\);/
  );
  assert.match(
    source,
