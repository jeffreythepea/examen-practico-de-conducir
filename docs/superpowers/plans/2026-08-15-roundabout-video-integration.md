...
**Date:** 2026-08-15
**Status:** Shipped and verified on Jeffrey's iPad 2026-08-15 — the scene,
the four clips and the gold-glyph suppression all confirmed working on device.
The checklist below was closed out in bulk at that point, on the evidence of
the shipped result rather than a line-by-line re-verification.
...
- [x] Jeffrey reviews all four clips for route, lane side, car scale, heading,
  timing, splitter clearance, and perspective.
- [x] Apply only requested visual corrections to the prototype render scripts.
- [x] Record SHA-256 checksums of the approved PNG and MP4 files.
- [x] Re-encode production MP4s using the repository's existing H.264,
  `yuv420p`, and fast-start conventions.
- [x] Extract a first-frame WebP poster for every clip; verify each poster is
  visually identical to the clip's opening frame.
- [x] Convert the approved still to the production WebP scene without changing
...
- [x] Register `roundabout-four-photo-v3` with bilingual alt text describing
  one entry branch, three numbered exits, and physical lane separators.
- [x] Register scene/result clips for `roundabout-exit-1`,
  `roundabout-exit-2`, `roundabout-exit-3`, and
  `roundabout-change-direction`.
- [x] Add `roundabout-v2` to the command-to-scene clip lookup without changing
  its stable surface ID.
- [x] Use measured MP4 durations in the registry; do not hardcode one shared
  duration.
- [x] Preserve the existing session-local clip-failure latch and static
  fallback behavior.
- [x] Prove every registered asset and poster exists, is decodable, and has
...
- [x] Point new `roundabout-v2` generations at `roundabout-four-photo-v3`.
- [x] Expose exactly four physical response targets: first, second, third, and
  return-through-entry.
- [x] Calibrate the fourth target to the outbound half of the bottom branch;
  it must not overlap the inbound half or the separator.
- [x] Keep legacy four-/five-exit generation code only where required to
  resolve an already-saved retired command; it must be unreachable from new
  session selection.
- [x] Update bilingual target labels so the return target reads as a change of
  direction, not a numbered fourth exit.
- [x] Prove every clip-backed correct reveal suppresses the gold glyph and
...
- [x] Keep `c-sentido` / `change-direction` / `u-turn-v1` unchanged.
- [x] Add a new stable command ID such as `c-sentido-rotonda`, with a unique
  action/result ID `roundabout-change-direction`, surface `roundabout-v2`, and
  unique phrasing IDs.
- [x] Use Spanish examiner instructions equivalent to the existing Cambio de
  sentido language; add English meanings for every phrase.
- [x] Reuse already-approved Spanish audio bytes by checksum where wording,
  voice, and speed are identical, while retaining distinct stable phrasing and
  audio-variant records for the new command.
- [x] Include the new action in the Roundabout circuit theme and normal driving
  selection.
- [x] Prove attempts, mastery, replay, persistence, and active-session restore
...
- [x] Mark `c-rot4` and `c-rot5` inactive for all newly constructed sessions.
  Do not rename, reuse, or reinterpret either ID.
- [x] Centralize the active-command filter so free practice, themes,
  recommendations, targeted practice, accomplishments, and release counts all
  exclude retired records.
- [x] Continue resolving historical attempts and any already-persisted active
...
- [x] Remove the five-exit scene from active distribution and from claims in
  bilingual UI/release documentation.
- [x] Update Roundabout circuit copy from “both layouts” to the new single
...
- [x] Exercise correct unaided and assisted answers for all four roundabout
  outcomes with Road movement On.
- [x] Confirm each reveal retains its reading beat, then transitions into the
  matching clip and auto-advances from the clip's measured duration.
- [x] Confirm wrong answers, timeouts, Mock/session-end reveal, Road movement
...
- [x] Confirm the scene still, clip first frame, and cruise transition have no
  visible framing jump.
- [x] Verify byte-range serving, offline packaging, and iPadOS Safari playback
...
- [x] Add a deterministic audit that walks every active driving command,
  ...
  clip is registered.
- [x] Group the report by reusable scene/result pair rather than phrasing, so
  one approved clip can cover every command variant with the same visual
  action.
- [x] Record a checked-in backlog with command ID, action ID, surface ID,
  scene/result pair, required motion, estimated clip reuse, and review status.
- [x] Produce clips in lowest-cost batches: shared junction/manoeuvre scenes
  first, unique scenes last. Review one representative clip from each new
  visual family before generating its siblings.
- [x] Register approved clips through `TURN_CLIPS`; do not couple scoring or
  command semantics to video availability.
- [x] Keep a static route—not an animated gold glyph—as the accessible fallback
  for reduced motion, Road movement Off, or video failure.
- [x] Once the audit reports zero active normal-path glyph questions, remove
...
- [x] `npm test`
- [x] `git diff --check`
- [x] Bilingual AI-voice disclosure remains visible.
- [x] No credential or provider secret exists in repository or browser files.
- [x] Stable command/action/phrasing IDs and provenance are preserved.
- [x] Manual English and Spanish review on a physical iPad or the accepted
  simulator target, including all four roundabout clips and one forced-failure
  fallback.
- [x] Jeffrey reviews all changes before any commit or push.
